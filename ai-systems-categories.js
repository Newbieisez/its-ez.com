(() => {
  ...
  const outcomeGroups = [
    {id:'pipeline',n:'01',title:'Create Pipeline',summary:'Find the right accounts, detect buying signals, enrich data and turn research into qualified outreach.',examples:'ICP · signals · enrichment · prospecting',match:p=>p.cats.includes('research')||['clay','apollo','zoominfo','6sense','common-room','usergems','bardeen','rows','reply','unify','regie-ai','strama-ai','amplemarket','landbase','persana-ai','cognism','lusha','seamless-ai','vibe-prospecting'].includes(p.id)},
    {id:'sales',n:'02',title:'Run the Sales Motion',summary:'Engage buyers, qualify opportunities, manage pipeline, progress deals and improve close execution.',examples:'outreach · MEDDPICC · pipeline · close',match:p=>p.cats.includes('revenue')},
    {id:'meetings',n:'03',title:'Improve Customer Conversations',summary:'Prepare for meetings, capture buyer evidence, update CRM and coach from what actually happened.',examples:'meeting AI · conversation intelligence · coaching',match:p=>p.cats.includes('coaching')||['granola','avoma','fathom','fireflies'].includes(p.id)},
    {id:'enablement',n:'04',title:'Enable & Train the Team',summary:'Build onboarding, learning, role-play, certification, reinforcement and field activation systems.',examples:'enablement · LMS · role-play · readiness',match:p=>p.cats.includes('enablement')||p.cats.includes('learning')},
    {id:'content',n:'05',title:'Create Better Content',summary:'Turn approved knowledge into courses, decks, video, audio, job aids and buyer-ready assets faster.',examples:'courses · video · decks · AI content',match:p=>p.cats.includes('creation')},
    {id:'knowledge',n:'06',title:'Find Trusted Knowledge',summary:'Make company knowledge searchable, grounded and useful inside the moments where teams need answers.',examples:'enterprise search · knowledge · grounded AI',match:p=>p.cats.includes('knowledge')},
    {id:'automation',n:'07',title:'Automate the Work',summary:'Connect triggers, agents, approvals, routing and system updates without creating invisible chaos.',examples:'workflows · agents · integrations · orchestration',match:p=>p.cats.includes('automation')},
    {id:'revops',n:'08',title:'Run CRM & Revenue Ops',summary:'Improve data quality, forecasting, routing, deal inspection, attribution and operating visibility.',examples:'CRM · RevOps · forecasting · attribution',match:p=>p.cats.includes('crm')||p.cats.includes('revops')},
    {id:'projects',n:'09',title:'Run Programs & Projects',summary:'Plan launches, coordinate cross-functional work, automate status and keep execution visible.',examples:'projects · launches · work management',match:p=>p.cats.includes('project')},
    {id:'partners',n:'10',title:'Scale Partners & Ecosystems',summary:'Recruit, onboard, accredit, activate, co-sell and measure partner contribution across the ecosystem.',examples:'PRM · partner learning · co-sell · ecosystem',match:p=>p.cats.includes('partner')}
  ];
  ...
  if(!document.getElementById('ai-systems-category-style')){
    const style=document.createElement('style');style.id='ai-systems-category-style';style.textContent=`
      ...
      @media(max-width:1180px){.outcome-grid{grid-template-columns:repeat(3,minmax(0,1fr))}.outcome-card{min-height:205px}}@media(max-width:820px){.outcome-navigator{padding:16px;border-radius:20px}.outcome-intro,.tool-library-subhead{align-items:flex-start;flex-direction:column}.outcome-intro p,.tool-library-subhead p{text-align:left}.outcome-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.browse-all-row{align-items:flex-start;flex-direction:column}.tool-library-subhead{margin-bottom:16px}}@media(max-width:560px){.outcome-grid{grid-template-columns:1fr}.outcome-card{min-height:0}.outcome-card strong{margin-top:20px}.outcome-examples{margin-top:16px}.outcome-navigator{margin-bottom:30px}}

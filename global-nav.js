(() => {
  ...
  .ez-global-links>a{position:relative;display:flex;align-items:center;min-height:40px;padding:0 8px;border:0;border-radius:8px;background:transparent;color:#d5d5d8!important;text-decoration:none!important;font:800 10.5px/1 Arial,Helvetica,sans-serif!important;letter-spacing:0!important;white-space:nowrap!important;transition:background .18s ease,color .18s ease}
  ...
  function installAiSystemMapOverride(){
    const path=location.pathname.toLowerCase();
    ...
    if(!document.getElementById('ez-ai-system-map-cleanup')){
      ...
      style.textContent=`
        .page-map-toggle{z-index:9991!important}
        .page-map{z-index:9990!important}
        @media(min-width:721px){
          .page-map{display:none!important;left:44px!important;top:50%!important;bottom:auto!important;width:224px!important;max-height:min(72vh,620px)!important;overflow:auto!important;transform:translateY(-50%)!important}
          .page-map.is-open{display:block!important}
          .page-map-toggle{display:flex!important;align-items:center!important;justify-content:center!important;left:0!important;top:50%!important;bottom:auto!important;width:34px!important;min-width:34px!important;min-height:124px!important;padding:10px 6px!important;border-radius:0 12px 12px 0!important;transform:translateY(-50%)!important;writing-mode:vertical-rl!important;text-orientation:mixed!important;font-size:9px!important;letter-spacing:.11em!important}
        }
        @media(max-width:720px){
          .page-map{display:none!important;left:12px!important;right:auto!important;top:auto!important;bottom:68px!important;width:min(230px,calc(100vw - 24px))!important;transform:none!important}
          .page-map.is-open{display:block!important}
          .page-map-toggle{display:flex!important;align-items:center!important;justify-content:center!important;left:12px!important;top:auto!important;bottom:12px!important;width:auto!important;min-width:0!important;min-height:46px!important;padding:0 16px!important;border-radius:999px!important;transform:none!important;writing-mode:horizontal-tb!important}
        }
      `;
      document.head.appendChild(style);
    }
  ...
  function fixMusicSoundCloud(){
    ...
    fetch('https://soundcloud.com/oembed?format=json&maxheight=320&color=ef1717&auto_play=false&show_comments=false&url='+encodeURIComponent(trackUrl),{mode:'cors'})
      ...
      .then(data=>{
        if(!data||!data.html)throw new Error();
  ...
  function loadPageExtensions(){
    ...
    if(path.endsWith('/ai-systems.html') || path.endsWith('/ai-systems')){
      if(!document.querySelector('script[data-ez-ai-systems-extension]')){
        ...
        s.src='/ai-systems-extended.js?v=20260907-1';
        s.dataset.ezAiSystemsExtension='true';
        document.head.appendChild(s);
      }
      if(!document.querySelector('link[data-ez-ai-systems-ratings-style]')){
        const l=document.createElement('link');
        l.rel='stylesheet';
        l.href='/ai-systems-ratings.css?v=20260908-1';
        l.dataset.ezAiSystemsRatingsStyle='true';
        document.head.appendChild(l);
      }
      if(!document.querySelector('script[data-ez-ai-systems-ratings]')){
        const r=document.createElement('script');
        r.src='/ai-systems-ratings.js?v=20260908-1';
        r.dataset.ezAiSystemsRatings='true';
